...
const API_BASE = "http://localhost:4000/api/education";
...
describe("/api/education", () => {
  ...
  describe("POST", () => {
    it("returns 503 when not configured", async () => {
      ...
      const req = createPOSTRequest({ institution: "Test" }, API_BASE);
  ...
  describe("PUT", () => {
    it("returns 400 when id missing", async () => {
      ...
      const req = createPUTRequest({ institution: "No id" }, API_BASE);
    ...
    it("returns success on valid update", async () => {
      ...
      const { PUT } = await import("@/app/api/education/route");
      const req = createPUTRequest({ id: "1", institution: "Updated" }, API_BASE);
      const res = await PUT(req);
      const data = await res.json();
      expect(res.status).toBe(200);
      expect(data.success).toBe(true);
    ...
    it("returns 500 on DB update error", async () => {
      ...
      const { PUT } = await import("@/app/api/education/route");
      const req = createPUTRequest({ id: "1", institution: "X" }, API_BASE);
      const res = await PUT(req);
      expect(res.status).toBe(500);
      const data = await res.json();
