...
describe("/api/services", () => {
  ...
  describe("GET", () => {
    ...
    it("returns DB data when configured", async () => {
      const rows = [{ id: "s1", title: "Backend Dev", display_order: 1 }];
  ...
  describe("POST", () => {
    it("returns 503 when not configured", async () => {
      ...
      const req = createPOSTRequest({ title: "Test" }, API_BASE);
    ...
    it("creates record with valid auth", async () => {
      const row = { id: "1", title: "Test", description: "desc", icon_name: "Server", display_order: 1 };
