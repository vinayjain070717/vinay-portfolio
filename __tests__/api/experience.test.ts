...
const ROUTE = "@/app/api/experience/route";
...
describe("/api/experience", () => {
  ...
  describe("GET", () => {
    ...
    it("returns DB data when configured", async () => {
      const dbRows = [{ id: "1", company: "Test", display_order: 1 }];
  ...
  describe("POST", () => {
    it("returns 503 not configured", async () => {
      ...
      const req = createPOSTRequest({ company: "Acme" }, API_URL);
    ...
    it("returns success", async () => {
      const inserted = { id: "exp-1", company: "Acme", display_order: 1 };
      ...
      const { POST } = await import(ROUTE);
      const req = createPOSTRequest({ company: "Acme", display_order: 1 }, API_URL);
      const res = await POST(req);
    ...
    it("returns 500 on DB error", async () => {
      ...
      const { POST } = await import(ROUTE);
      const req = createPOSTRequest({ company: "Bad" }, API_URL);
      const res = await POST(req);
      const body = await res.json();
  ...
  describe("PUT", () => {
    it("returns 400 no id", async () => {
      ...
      const req = createPUTRequest({ company: "X" }, API_URL);
      const res = await PUT(req);
      const body = await res.json();
      expect(res.status).toBe(400);
      expect(body.error).toBe("Experience id is required");
    });
  });
  ...
  describe("DELETE", () => {
    ...
    it("returns success", async () => {
      ...
      const { DELETE } = await import(ROUTE);
      const req = createDELETERequest(`${API_URL}?id=exp-1`);
      const res = await DELETE(req);
      const body = await res.json();
      expect(res.status).toBe(200);
