...
const API_BASE = "http://localhost:4000/api/journey";
...
describe("/api/journey", () => {
  ...
  describe("POST", () => {
    ...
    it("returns 503 when not configured", async () => {
      ...
      const { POST } = await import("@/app/api/journey/route");
  ...
  describe("PUT", () => {
    it("returns 400 when id missing", async () => {
      ...
      const { PUT } = await import("@/app/api/journey/route");
      const req = createPUTRequest({ title: "No id" }, API_BASE);
      const res = await PUT(req);
      expect(res.status).toBe(400);
      const data = await res.json();
      expect(data.error).toBe("Journey event id is required");
    });
    ...
    it("returns success on valid update", async () => {
      ...
      const { PUT } = await import("@/app/api/journey/route");
      const req = createPUTRequest({ id: "1", title: "Updated" }, API_BASE);
      const res = await PUT(req);
      const data = await res.json();
      expect(res.status).toBe(200);
      expect(data.success).toBe(true);
    ...
    it("returns 500 on DB update error", async () => {
      ...
      const { PUT } = await import("@/app/api/journey/route");
      const req = createPUTRequest({ id: "1", title: "X" }, API_BASE);
      const res = await PUT(req);
      expect(res.status).toBe(500);
      const data = await res.json();
  ...
  describe("DELETE", () => {
    it("returns 400 when id query param missing", async () => {
      ...
      const { DELETE } = await import("@/app/api/journey/route");
